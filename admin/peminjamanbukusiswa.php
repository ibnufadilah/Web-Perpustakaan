...
<br>
  ...
  <table id="table" class="table table-bordered">
   ...
   $baca= "SELECT * FROM peminjaman_siswa   order  by id_pinjamsiswa ASC";
   ...
   if (mysqli_num_rows($tampil)>0) {
    ...
    while($row=mysqli_fetch_assoc($tampil)){
      ...
      <td>$row[jumlah]</td>
      <td>$row[status] oleh $row[nama]</td>
     <td>
                  <button type='button' class='btn btn-success' data-toggle='modal' data-target='#perpanjang-$row[id_pinjamsiswa]'>
      perpanjang </button> <br><br></td>"
      ?>
...
<?php if ($row['status'] == 'Pinjam'){
  ...
  ?>
      ...
      <td>
       <a href="kembali_buku.php?id_pinjamsiswa=<?php echo $row['id_pinjamsiswa']; ?>" class="btn btn-primary"> Kembalikan Buku </a> <br><br>
                 </td> <?php }
                  else{echo "<td>Telah Dikembalikan</td>";}
                   ?>
                   <td>
                     <?php echo "<button type='button' class='btn btn-secondary' data-toggle='modal' data-target='#hilang-$row[id_pinjamsiswa]'>
      Ubah Status </button> "; ?><br><br>
                   </td>
        <?php echo"        
       <td>
       <a class='btn btn-danger' href=delete_pinjam.php?id_pinjamsiswa=$row[id_pinjamsiswa]> Delete</a><br><br></td>          
      ...
      ?>
        ...
        <div class="modal fade" id="perpanjang-<?php echo $row['id_pinjamsiswa']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLongTitle">UPDATE PERPANJANG PEMINJAMAN BUKU</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form action="aksi_perpanjang.php" method="post">
              <input type="hidden" name="id" value="<?php echo $row['id_pinjamsiswa']; ?>"> 
              <div class="form-group">
                <label for="email">Tanggal Kembali :</label>
                <input type="date" class="form-control" id="email" name="tgl_kembali" value="<?php echo $row['tgl_kembali']; ?>">
              </div>
            </div>
     ...
     <div class="modal fade" id="hilang-<?php echo $row['id_pinjamsiswa']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLongTitle">UPDATE HILANG ATAU RUSAK BUKU</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <form action="aksi_hilang.php" method="post">
              <input type="hidden" name="id" value="<?php echo $row['id_pinjamsiswa']; ?>"> 
              <div class="form-group">
                <label for="email">HILANG ATAU RUSAK :</label>
